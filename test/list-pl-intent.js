...
describe('Testing listPlaylists intent', function() {
  ...
  before(done => {
    index.handler(
      _.merge(event, { request: { intent: { name: 'ListPlaylists' } } }),
      ctx
    );
  ...
  describe('The response is correct', () => {
    it('outputSpeech is list of Playlists', () => {
      ...
      expect(speechResponse.response.outputSpeech.ssml).to.match(/playlists/);
    });

    it('card is list of Playlists', () => {
      expect(speechResponse.response.card.type).to.equal('Simple');
      expect(speechResponse.response.card.title).to.match(/List Playlists/);
      expect(speechResponse.response.card.content).to.match(/playlists/);
    });
